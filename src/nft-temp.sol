...
contract NextGen is ERC1155, Ownable {
    ...
    string private constant BASE_URI = "https://ipfs.filebase.io/ipfs/";
    ...
    function mint(uint256 count) external {
        require(
            count <= numLimitPerMint,
            string(
                abi.encodePacked(
                    "max minting ",
                    string(abi.encodePacked(numLimitPerMint)),
                    " tokens each time."
                )
            )
        );
        ...
        require(
            _total + count <= _maxLimit,
            string(
                abi.encodePacked(
                    "maximum minting amount is limited to ",
                    string(abi.encodePacked(_maxLimit))
                )
            )
        ); //?
    ...
    function compose(
        ...
        require(
            _targetId > 0 && _targetId < 4,
            "target level must be greater than zero and less than four"
        );
        ...
        require(
            ...
            string(
                abi.encodePacked(
                    "maximum composing amount is limited to ",
                    string(abi.encodePacked(_maxLimit))
                )
            )
        );
    ...
    function uri(
        ...
        return string(abi.encodePacked(BASE_URI, cid));
    }
}
...
contract PledgeNano is Ownable, ReentrancyGuard {
    ...
    function speedForParent(uint256 _amount) private {
        ...
        if (hasParent[msg.sender] == true && isStaked[_parent] == true) {
            ...
            for (uint256 i = 0; i < parentStakeIds.length; i++) {
                ...
                (, , , , , , uint256 norelease, , , , ) = getUserStakeInfo(
                    _parent,
                    parentStakeId
                );
                ...
                if (norelease > 0) {
                    if (norelease >= fixSpeedAmount * _amount) {
                        ...
                        parentLock.norelease =
                            parentLock.ntAmounts -
                            parentLock.hasreleaseall;
                    } else {
                    ...
                    isSpeedCountMarked[_parent][parentStakeId][
    ...
    function accumulateRealease2(
        address _player,
        uint256 _stakeId
    ) public view returns (uint256) {
        LockInfo storage userLockInfo = stakes[_player][_stakeId];
        uint256 daysPassed = (block.timestamp - userLockInfo.releaseStartTime) /
            releaseInterval;
        uint256 accumulatedRelease = 0;

        if (
            daysPassed < lockDuration &&
            userLockInfo.norelease >= fixSpeedAmount
        ) {
            accumulatedRelease =
                (daysPassed * releaseNtTokenPerDay * userLockInfo.ntAmounts) /
                100 /
                10 ** 18 +
        ...
        if (
            daysPassed < lockDuration &&
            userLockInfo.norelease < fixSpeedAmount &&
            isSpeedCountMarked[_player][_stakeId][userLockInfo.speedCount] ==
            false
        ) {
            accumulatedRelease =
                (daysPassed * releaseNtTokenPerDay * userLockInfo.ntAmounts) /
    ...
    function cycle(
        ...
        uint256 daysPassed = (block.timestamp - lock.releaseStartTime) /
            releaseInterval;
    ...
    function claimNtToken() external nonReentrant {
        ...
        for (uint256 i = 0; i < stakeIds.length; i++) {
            ...
            (, , , , , , , , uint256 waitforclaim, , ) = getUserStakeInfo(
                msg.sender,
                stakeId
            );
        ...
        userClaimInfo[msg.sender].push(
            ClaimInfo(totalClaimedAmount, block.timestamp)
        );
    ...
    function getAllStakeIdsByAddress(
        address _address
    ) public view returns (uint256[] memory) {
        return accountHasStakeIdAmounts[_address];
    }

    function getAllStakeidsAmounts(
        address _address
    ) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < stakeIds.length; i++) {
            ...
            (, , uint256 ntAmounts, , , , , , , , ) = getUserStakeInfo(
                _address,
                stakeId
            );
    ...
    function getAllChildsByParent(
        address _parent
    ) external view returns (ChildInfo[] memory childrenInfo) {
        address[] storage childAddresses = parentToChilds[_parent];
        ChildInfo[] memory childrenInfoArray = new ChildInfo[](
    ...
    function getTotalHasReleaseAllAmount(
        address user
    ) public view returns (uint256) {
        uint256 totalAmount = 0;
        ...
        for (uint256 i = 0; i < stakeIds.length; i++) {
            uint256 stakeId = stakeIds[i];
            (, , , , , , , uint256 hasreleaseall, , , ) = getUserStakeInfo(
                user,
                stakeId
            );
    ...
    function getTotalWaitforclaimAmount(
        address user
    ) public view returns (uint256) {
        uint256 totalAmount = 0;
        uint256[] memory stakeIds = accountHasStakeIdAmounts[user];
    ...
    function getTotalhasclaimallAmount(
        address user
    ) public view returns (uint256) {
        uint256 totalAmount = 0;
        uint256[] memory stakeIds = accountHasStakeIdAmounts[user];
        ...
        for (uint256 i = 0; i < stakeIds.length; i++) {
            uint256 stakeId = stakeIds[i];
            (, , , , , , , , , uint256 hasclaimallamount, ) = getUserStakeInfo(
                user,
                stakeId
            );
    ...
    function getSpeedCountByAddress(
        address user
        ...
        for (uint256 i = 0; i < stakeIds.length; i++) {
            uint256 stakeId = stakeIds[i];
            (, , , , , , , , , , uint256 speedCount) = getUserStakeInfo(
                user,
                stakeId
            );
    ...
    function emergencyWithdraw() external nonReentrant onlyOwner {
        ntCoinContract.transfer(
            msg.sender,
            ntCoinContract.balanceOf(address(this))
        );
    }

    function setNanoGenContract(
        address _nanoGenContract
    ) external nonReentrant onlyOwner {
        nanoGenContract = INanoGen(_nanoGenContract);
    }

    function setNTCoinContract(
        address _ntCoinContract
    ) external nonReentrant onlyOwner {
        ntCoinContract = INTCoin(_ntCoinContract);
    }

    function setBusdContract(
        address _busdContract
    ) external nonReentrant onlyOwner {
        busdContract = IERC20(_busdContract);
    }
...
contract NanoGen is ERC1155, Ownable, ReentrancyGuard {
    ...
    function Manage(uint256 amount) external onlyManager nonReentrant {
        ...
        ntCoinContract.transferFrom(
            msg.sender,
            address(nanoStakingContract),
            amount * 10000 * 1000000000000000000
        );
    ...
    function setNanoStakingContract(
        address _nanoStakingContract
    ) external onlyOwner nonReentrant {
    ...
    function setMarketAccount(
        address _marketAccount
    ) external onlyOwner nonReentrant {
